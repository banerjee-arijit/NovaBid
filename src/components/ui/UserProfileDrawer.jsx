...
import ReactToaster from "./Toaster";
import { useNavigate } from "react-router";

const UserProfileDrawer = ({ open, setOpen }) => {
  const [userName, setUserName] = useState("Arijit");
  const [userEmail, setUserEmail] = useState("arijit@gmail.com");
  const [isEditing, setIsEditing] = useState(false);
  const [myProducts, setMyProducts] = useState(12);
  const [totalRevenue, setTotalRevenue] = useState(10500);
  ...
  const handleUserBtnClick = (label) => {
    if (label === "Logout") {
  ...
  const handleLogout = async () => {
    toast(`😔${name} Come back soon—your next big win awaits!`);
                ...
                <div className="space-y-2">
                  {menuItems.map((item, index) => (
                    <motion.button
                      ...
                      onClick={() => handleUserBtnClick(item.label)}
                    >
